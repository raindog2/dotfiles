...
" This is the command to print in vim:
" :w !lp
...
set autochdir           " Change the current directory to that of the current file
set noshowmode          " Don't show what mode you are in (airline does that)
...
set textwidth=0         " Do not insert line breaks automatically
set wrapmargin=0        " Do not insert line breaks automatically
set laststatus=2        " Keep statusbar on always
...
set timeoutlen=3000     " Timeout for keymappings: 3 seconds
set ttimeoutlen=100     " Timeout for keycodes: 0.1 seconds
"setlocal fo+=aw        " Help with wrapping and line breaks in Mutt
...
"Enable colors if terminal supports colors
"if t_Co > 1
"    syntax enable
"endif
...
" set movemnent keys for colemak
" up: 
noremap u k    
"down:
noremap e j
...
"right:
noremap i l
"need to use h as insert command instead of i
noremap h i
"need to use k as undo (kill) command instead of u
noremap k u
"need to use N as find next command instead of n
noremap N n

" Move down one visual line instead of one actual line
" ***These are set for Colemak***
noremap <buffer> <silent> u gk
noremap <buffer> <silent> e gj
...
" Make inserting inside markers more convenient
...
if filereadable("/etc/vim/vimrc.local")
 ...
 """""""""""REFERENCE""""""""""
"For all the cterm color codes, see http://www.pixelbeat.org/docs/terminal_colours/
